...
const followButtonClassName =
  "inline-block cursor-pointer rounded-[2px] border border-teak bg-transparent px-[22px] py-2.5 font-body text-[0.76rem] font-semibold uppercase tracking-[0.07em] text-teak transition-all duration-200 hover:bg-teak hover:text-ink";
...
const JOURNEY_STEPS = [
  {
    title: "Consultation",
    description:
      "Free, the same complimentary consultation offered before every treatment plan.",
  },
  {
    title: "Personalized plan",
    description: "Built around what you want to see, not a fixed package.",
  },
  {
    title: "Treatment",
    description: "Performed by Michelle or a licensed member of the team.",
  },
  {
    title: "6-month milestone",
    description:
      "Paid membership tiers include a free bonus service at 6 months, worth $400–$1,000 depending on plan.",
  },
  {
    title: "Goal met",
    description: "Reviewed together, adjusted if needed, always natural.",
  },
] as const;
...
const CREDENTIALS = [
  ...
  "Regular attendee at national aesthetics conferences & trainings",
] as const;
...
export default function Home() {
  return (
    <>
      <section className="relative overflow-x-hidden bg-ink">
        ...
        <div className="relative z-10 mx-auto grid min-h-[calc(100vh-72px)] max-w-[1200px] grid-cols-1 items-center gap-10 px-4 py-12 md:grid-cols-2 md:gap-[60px] md:px-6 md:py-20">
          <div className="text-center md:text-left">
            ...
            <h1 className="mb-7 font-display text-[clamp(2.6rem,5.5vw,4.4rem)] leading-[1.12] text-warm-white">
              Tell us the goal.{" "}
              <span className="italic text-teak">We&apos;ll build the plan</span>{" "}
              to reach it.
            </h1>

            <p className="mx-auto mb-10 max-w-[480px] text-base leading-[1.7] text-warm-white/60 md:mx-0">
              Michelle Tio-Nguyen, FNP-C, brings 10 years of master-level
              injectables, advanced lasers, and personalized aesthetic
              medicine to every treatment plan at 8961 W. Sahara Ave.
            </p>
            ...
            <p className="mt-5 text-[0.8rem] text-warm-white/50">
              Membership from <strong className="text-teak">$99/mo</strong>:
              10&ndash;15% off all services · Credits always roll over
            </p>
          </div>

          <aside
            className="relative rounded border border-teak/15 bg-teak/[0.05] px-6 py-8 md:px-10 md:py-12"
            aria-label="Practice highlights"
          >
            <Image
              src="/images/logo-emblem.png"
              alt="Beauty Goals LV emblem"
              width={96}
              height={96}
              className="absolute -right-4 -top-6 h-16 w-16 md:-right-6 md:-top-8 md:h-24 md:w-24"
            />

            <p className="mb-6 font-mono text-[0.7rem] uppercase tracking-[0.12em] text-teak">
              Why clients trust the plan
            </p>
            <ul className="flex flex-col">
              {HERO_MILESTONES.map((milestone, index) => (
                <li
                  key={milestone.label}
                  className={cn(
                    "flex items-baseline gap-4 py-3.5",
                    index > 0 && "border-t border-teak/15",
                  )}
                >
                  <span className="min-w-[3.2ch] font-mono text-[1.7rem] text-teak">
                    {milestone.value}
                  </span>
                  <span className="text-[0.85rem] text-warm-white/60">
                    {milestone.label}
                  </span>
                </li>
              ))}
      ...
      <section className="section-padding bg-warm-white">
        <div className="mx-auto max-w-[1200px]">
          <div className="mb-11 max-w-[640px]">
            <p className="mb-4 inline-flex items-center gap-2 font-mono text-[0.72rem] uppercase tracking-[0.12em] text-teak-dark">
              <Tick className="text-moss" />
              How a goal becomes a result
            </p>
            <h2 className="h2-section text-body">
              Five steps. No guesswork in between.
            </h2>
          </div>

          <ul className="grid grid-cols-1 gap-8 min-[701px]:grid-cols-5 min-[701px]:gap-4">
            {JOURNEY_STEPS.map((step, index) => (
              <li key={step.title}>
                <div
                  className={cn(
                    "mb-4 flex h-6 w-6 items-center justify-center rounded-full border-2",
                    index === JOURNEY_STEPS.length - 1
                      ? "border-teak bg-teak"
                      : "border-teak bg-warm-white",
                  )}
                >
                  <Tick
                    className={cn(
                ...
                <h3 className="mb-1.5 font-display text-base text-body">
                  {step.title}
                </h3>
                <p className="text-[0.84rem] leading-[1.55] text-muted">
                  {step.description}
                </p>
              </li>
            ))}
        ...
        <ul className="mx-auto flex max-w-[1200px] flex-wrap items-center justify-center gap-6 md:justify-between md:gap-4">
          {SOCIAL_PROOF.map((item) => (
            <li key={item.label} className="min-w-0 flex-1 basis-[45%] text-center sm:basis-auto sm:min-w-[120px]">
              ...
              <p className="mt-0.5 font-body text-[0.7rem] uppercase tracking-[0.06em] text-warm-white/[0.45]">
                {item.label}
              </p>
            </li>
          ))}
            ...
            {SERVICES.map((service) => (
              <li key={service.title}>
                <article className="group flex h-full flex-col bg-white p-8 transition-colors duration-200 hover:bg-warm-white">
                  ...
                  <p className="flex items-center gap-1.5 text-[0.78rem] font-semibold tracking-[0.04em] text-moss">
        ...
        <div className="mx-auto grid max-w-[1200px] grid-cols-1 items-center gap-10 md:grid-cols-2 md:gap-20">
          ...
          <div>
            ...
            <h2 className="mb-5 h2-section text-body">
              Master injector. Skin expert. Your partner in the goal.
            </h2>
            ...
            <ul className="flex flex-col gap-2.5">
              {CREDENTIALS.map((credential) => (
                ...
                >
                  <Tick className="mt-0.5 h-4 w-4 text-moss" />
                  {credential}
                </li>
              ))}
            </ul>
      ...
      <section className="section-padding bg-warm-white">
        <div className="mx-auto max-w-[1200px]">
          <SectionEyebrow>Goals, Met</SectionEyebrow>

          <h2 className="mb-3 h2-section text-body">
            See what reaching a goal actually looks like.
          </h2>

          <p className="mb-10 max-w-[640px] text-[1.1rem] leading-relaxed text-muted">
            Every photo is a real Beauty Goals LV client. No filters, no
            stock images.
          </p>
          ...
          <ul className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {BEFORE_AFTER.map((item) => (
              <li key={item.treatment}>
                <article className="overflow-hidden rounded border border-teak/[0.12] bg-ink">
                  <div className="flex min-h-[200px] bg-gradient-to-br from-[#111111] to-[#1f1a14]">
                    ...
                    <div
                      className="flex min-h-[200px] flex-1 items-center justify-center px-2 py-[60px] text-center text-[0.65rem] uppercase tracking-[0.1em] text-warm-white/30"
                      role="img"
                    ...
                    {/* TODO: Replace with <Image> from next/image when real photos are available */}
                    <div
                      className="flex min-h-[200px] flex-1 items-center justify-center border-l border-teak/20 px-2 py-[60px] text-center text-[0.65rem] uppercase tracking-[0.1em] text-warm-white/30"
                      role="img"
                  ...
                  <div className="flex items-center justify-between gap-3 bg-teak/[0.04] px-4 py-3.5">
                    <div>
                      <p className="text-[0.8rem] font-semibold text-warm-white">
                        {item.treatment}
                      </p>
                      <p className="mt-0.5 text-[0.72rem] text-warm-white/40">
                        {item.detail}
                      </p>
                    </div>
                    <span className="inline-flex shrink-0 items-center gap-1 whitespace-nowrap rounded-[2px] border border-teak/40 px-2 py-1 font-mono text-[0.62rem] uppercase tracking-[0.04em] text-teak">
      ...
      <section className="section-padding bg-ink">
        <div className="mx-auto max-w-[1200px]">
          ...
          <h2 className="mb-12 h2-section text-warm-white">
            ...
            {TESTIMONIALS.map((testimonial) => (
              <li key={testimonial.name}>
                <article className="rounded border border-teak/15 bg-teak/[0.06] p-7">
                  ...
                  <blockquote className="mb-5 text-[0.88rem] italic leading-[1.7] text-warm-white/75">
                  ...
                  <footer className="flex items-center gap-2.5">
                    ...
                    <div>
                      <cite className="not-italic">
                        <p className="text-[0.82rem] font-medium text-warm-white">
                          {testimonial.name}
                        </p>
                        <p className="text-[0.72rem] text-warm-white/35">
                          {testimonial.detail}
                        </p>
                      </cite>
                    </div>
          ...
          <ul className="grid grid-cols-3 gap-[3px] md:grid-cols-6">
            {INSTAGRAM_TILES.map((label) => (
              <li key={label}>
                {/* TODO: Replace with <Image> from next/image when real photos are available */}
                <div
                  className="flex aspect-square cursor-pointer items-center justify-center border border-teak/[0.08] bg-gradient-to-br from-ink to-[#252015] p-2 transition-opacity duration-200 hover:opacity-80"
                  role="img"
                  aria-label={label}
                >
      ...
      <section className="section-padding bg-ink">
        <div className="mx-auto grid max-w-[1200px] grid-cols-1 items-start gap-12 md:grid-cols-2 md:gap-20">
          <div>
            ...
            <h2 className="mb-5 h2-section text-warm-white">
              Let&apos;s start your glow-up.
            </h2>

            <p className="mb-8 leading-[1.7] text-warm-white/[0.65]">
              Fill out the form and we&apos;ll be in touch within 24 hours.
              Prefer to call? We&apos;re at 702-625-3838 during business
              hours.
            </p>

            <address className="not-italic">
              <div className="flex items-start gap-3.5 border-b border-teak/15 py-4 text-[0.9rem] text-warm-white">
